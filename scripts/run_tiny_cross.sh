...
# Simple, reproducible Peg Solitaire TINY-CROSS experiment
...
RUN_ID="tiny_cross_smoke_$(date +%Y%m%d_%H%M%S)"

echo "[INFO] Starting tiny_cross run: ${RUN_ID}"
echo "[INFO] PYTHONPATH set to include: ${REPO_ROOT}/src"

python -m mdpmm.training.hydra_train \
  env=tiny_cross algo=dqn \
