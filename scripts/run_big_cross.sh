...
# Simple, reproducible Peg Solitaire BIG-CROSS experiment
...
RUN_ID="big_cross_smoke_$(date +%Y%m%d_%H%M%S)"

echo "[INFO] Starting big_cross run: ${RUN_ID}"
echo "[INFO] PYTHONPATH set to include: ${REPO_ROOT}/src"

python -m mdpmm.training.hydra_train \
  env=big_cross algo=dqn \
