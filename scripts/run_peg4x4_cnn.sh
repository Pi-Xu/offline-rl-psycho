...
RUN_ID="peg4x4_smoke_$(date +%Y%m%d_%H%M%S)"

echo "[INFO] Starting peg4x4 run: ${RUN_ID}"
...
python -m mdpmm.training.hydra_train \
  env=peg4x4 algo=dqn_cnn \
  train.train_episodes=2000 train.max_steps_per_episode=80 \
  eval.eval_episodes=20 train.eval_every=20 \
  train.device=cuda train.print_eval=true \
  train.render_gif=true \
  train.seed=42 \
  algo.epsilon.decay_steps=75000 \
  algo.replay_capacity=50000 \
  algo.epsilon.end=0.05 \
  algo.lr=1e-4 \
  paths.artifacts_dir=artifacts/models/peg/dqn_cnn \
  run_id="${RUN_ID}"

echo "[INFO] Done. Artifacts in artifacts/models/peg/dqn_cnn/${RUN_ID}"
